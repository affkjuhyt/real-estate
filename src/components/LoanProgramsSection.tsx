    ...
    <div className="bg-white py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex items-center gap-4 mb-12">
          <div className="h-[2px] bg-[#c99909] w-8"></div>
          <h1 className="text-[#c99909] text-2xl sm:text-2xl font-bold">{messages['loan_programs']['main_title']}</h1>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
            {loanPrograms.slice(0, 3).map((program, index) => (
              <div key={index} className="border border-gray-200 p-8 sm:p-10 flex flex-col rounded-lg items-center text-center shadow-sm hover:shadow-md transition-shadow">
                ...
                <h3 className="text-sm text-[#c99909] uppercase font-medium sm:text-base">{program.term}</h3>
                <h3 className="text-sm text-gray-600 mb-6 sm:text-base">{program.description}</h3>
                <Link 
                  href={program.link} 
                  className="mt-auto bg-[#c99909] text-white font-medium text-sm py-2 px-4 rounded hover:bg-[#e06a15] transition-colors"
                >
